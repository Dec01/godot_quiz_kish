...
var question_list = ["'Я всех замучить был бы рад, И от того я так невесел, Я в маске рыжей обезьяны, На праздник к вам попасть мечтал, Когда б не камень окаянный, ...'", 
"Минус какой песни сейчас играет фоном?", 
"'Он волен взять, И поменять, Строку и с ней, ...'", 
"Как называет песня которая, содержит строки: 'Его я усадил за стол, и он молчал, Ему чаю я налил, но снова кто-то постучал'?", 
"И восковых фигур, Прекрасен вид — ...",
"Зачем гонялся дурак?", 
"В какой маске путник шёл на праздник?", 
"Ведьма и ... ?", 
"Я дверь с улыбкой закрыл, Войти ему не дал, 'Меня ты не удивил ... '", 
"'Мы, анархисты, народ ... '", 
"Как звали охотника, что спал на чердаке?", 
"'Крестик на моей груди, На него ты погляди, Что в тебе способен он ...'"]

var answer_list = [["Что мне на голову упал", "Что мне на ногу упал", "Что на меня свалился", "Что мне на голову свалился"],
["Тайна хозяйки старинных часов", "Похороны панка", "Лесник", "Проказник скоморох"],
["Смысл темы всей", "Смысл песни всей", "Смысл всех идей", "Смысл произведения"],
["Два монаха в одну ночь", "Два вора и монета", "Два друга и разбойники", "Пивной череп"],
["Покой везде царит!", "Душа моя болит!", "Я всеми позабыт!", "Историю творит!"],
["Молнией", "Ветром", "Свободой", "Женой"],
["Обезьяны", "Вороны", "Короля", "Льва"],
["Осёл", "Леший", "Вурдалак", "Медведь"],
["И не такое я видал", "Твой фокус разгадал", "Заранее всё знал", "Я осознал, что спал"],
["не злой!", "простой!", "с душой!", "земной! Панки, хой!"],
["Себастьян", "Феодосий", "Парфён", "Сэмюэль"],
["Резко изменить?", "Резко заменить?", "Резко подменить?", "Сильно изменить?"]]
...
func _process(delta):
	if (question_number >= 0 or question_number < 11) and question_def == false:
		...
		$"%finish".show()
		$"%finish/%much_raund_finish".text = str(Global.pesnya_items)
		$"%back".disabled = true
...
func _on_answ_1_pressed():
	if question_answer == 1:
		...
		Global.pesnya_items += 1
		get_node("%menu_panel2/%much_raund").text = str(Global.pesnya_items)
...
func _on_exit_plus_pressed():
	Global.summ_items += 5
	Global.summ_items += Global.pesnya_items
	get_tree().change_scene("res://scenes/main.tscn")
